...
def generate_order_data():
    # 设置时间范围和数量
    start_time_create_early = datetime(2023, 12, 29, 0, 0, 0)
    end_time_create_early = datetime(2023, 12, 29, 7, 0, 0)

    start_time_create_late = datetime(2023, 12, 29, 7, 0, 1)
    end_time_create_late = datetime(2023, 12, 29, 18, 0, 0)

    start_time_create_final = datetime(2023, 12, 29, 18, 0, 1)
    end_time_create_final = datetime(2023, 12, 29, 23, 59, 59)

    # 每个时间段生成的个数
    num_intervals_create_early = 19
    num_intervals_create_late = 270
    num_intervals_create_final = 120
...
def save_to_excel(df, filename='455W_all_balck_time.xlsx'):
